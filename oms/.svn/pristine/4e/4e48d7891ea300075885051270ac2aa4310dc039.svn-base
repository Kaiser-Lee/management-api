...
public class MemberROPController extends BaseController {
	...
	@PassToken
	public ResultVO memberLoginOrRegister(HttpServletRequest request,@RequestBody MemberLoginRequestDTO requestDTO) {
		...
		// 1、先判断是否存在账号
		ResultVO<MemberIsExistsResp> resultVO = memberIsExists(requestDTO);
		if(!resultVO.getResultData().isExists()){
			// 2、不存在账号，注册（带登陆功能）
			MemberRegisterRequestDTO registerRequestDTO = new MemberRegisterRequestDTO();
			registerRequestDTO.setMobile(requestDTO.getMobile());
			registerRequestDTO.setVerificationCode(requestDTO.getPwd()); // 验证码
			return memberRegister(request, registerRequestDTO);

		}else{
			// 3、存在账号，登陆
			return memberLogin(request, requestDTO);
		}
	}
	...
	public ResultVO memberLogin(HttpServletRequest request,@RequestBody MemberLoginRequestDTO requestDTO) {
		...
		request.getSession().setAttribute(OmsConst.SESSION_MEMBER, resp.getMember());
		Map objMap = Maps.newConcurrentMap();
		objMap.put("member", resp.getMember());
		
		//绑定openId
        bindingOpenId(request, resp.getMember());
	 ...
	 * @param member
	 */
	private void bindingOpenId(HttpServletRequest request,Member member) {
		//TODO 从注册返回获取到memberId 添加绑定openid操作
		String openId = (String)request.getSession().getAttribute(OmsConst.SESSION_WX_OPEN_ID);
		if (!StringUtil.isEmpty(openId) && member != null) {

        	//先判断是否绑定过
            MemberBinding memberBinding= memberService.getMemberBinding(openId);
			if(memberBinding != null){
			    //判断会员名称和ID  是否相同   否：记录更新
                if(!StringUtil.equals(member.getMember_id(), memberBinding.getMemberId())
                        && !StringUtil.equals(member.getUname(), member.getUname())){
                    memberBinding.setMemberId(member.getMember_id());
                    memberBinding.setUname(member.getUname());
                    memberService.updateMemberBinding(memberBinding);
                }
                return; // 绑定过直接返回
			}
			//新增
			memberBinding = new MemberBinding();
            memberBinding.setMemberId(member.getMember_id());
	...
	public ResultVO<MemberIsExistsResp> memberIsExists(@RequestBody MemberLoginRequestDTO requestDTO) {
